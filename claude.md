...
- **Syringe block:** vertical sliders set Syringe-Height (B, 0–304 mm, Lower/Raise) and the plunger (C, 0–135 mm, absolute jog); the dispense/pullback amounts (mL, via a hand-measured mm-per-mL calibration) are sent as **relative C moves** (`G91` / `G1 C<mm>` / `G90`) — never `G92` on the homed C axis. (The Touch UI's copy of this block still sends the old `G92 E0` / `G1 E…` idiom, which the firmware silently ignores — pending port.)
