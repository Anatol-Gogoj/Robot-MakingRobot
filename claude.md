...
- **Syringe block** (identical in `RMR_Controller.html` and `RMR_Touch.html`): vertical sliders set Syringe-Height (B, 0–304 mm, 0 = up/home, Lower/Raise) and the plunger (C, 0–135 mm, 0 = fully open, absolute jog); the dispense/pullback amounts (mL, via a hand-measured mm-per-mL calibration) are sent as **relative C moves** — never `G92` on the homed C axis, so the soft endstops keep applying. Run does the full cycle: `G90` → `G1 B<height>` → `M400` → `G91` → `G1 C<mm>` (+C = dispense) → `M400` → optional `G1 C-<pullback mm>` → `M400` → `G90` → `G1 B0` → `M400`. The old `G92 E0` / `G1 E<mm>` form is silently ignored by the firmware (gotcha #1).
