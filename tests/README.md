...
| `syringe-dose.test.mjs` | — | The retuned Syringe block: exact command order (`M114` position query · lower · dose · `M400` · dwell · retract at its own feed · snap lift · raise), the dose / retract guards refusing with nothing sent, the stroke guard checking the fresh `M114` position rather than the readout (and refusing when the reply has no C), the purge and prime routines, `G90` still sent when a Stop or fault lands inside the relative-mode section (a Stop sends `M410` then `G90` in both UIs), barrel calibration from the bore, and the Run Log's preset → Process Sequencer apply path. |
