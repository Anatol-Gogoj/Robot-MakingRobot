...
- Elements register themselves in the stub when their `id` is assigned, so a
  ...
  Radio groups (a `name`, no `id`) are seeded too, with browser-style
  exclusivity, so `document.querySelector('input[name="stampSide"]:checked')`
  resolves — that is how the Stamp block reads its feeder side.
...
| `runlog-sync.test.mjs` | — | The shared `RMR-RUNLOG` block is identical in both pages (line endings aside, so a CRLF checkout passes) and each page carries its glue (containers, host object, every hook, and the Touch `tabNames` order). |
| `syringe-dose.test.mjs` | — | The retuned Syringe block: exact command order (`M114` position query · lower · dose · `M400` · dwell · retract at its own feed · raise; the B snap lift is gone), the dose / retract guards refusing with nothing sent (max retract defaults to 2 mm and a 0.5 mL drip-stopping pullback passes), the stroke guard checking the fresh `M114` position rather than the readout (and refusing when the reply has no C), the purge and prime routines, `G90` still sent when a Stop or fault lands inside the relative-mode section (a Stop sends `M410` then `G90` in both UIs), barrel calibration from the bore, and the Run Log's preset → Process Sequencer apply path. |
| `stamp-block.test.mjs` | — | The Stamp block in either UI: exact command order (travel · `M400` · gripper open · feeder advance · grab · transit · press · three-step retreat · dispose); the filter feeder advances toward MIN (`G1 A-`, the axis homes to MAX) once per pick in left-/right-only mode and once per L+R pair when alternating; the lift-Z guard refusing a lift at or below the press / approach depth before anything is sent, without consuming the alternating side; the filter-feeder jog buttons (Advance = `A-`, Retract = `A+`, locked until homed, refused while a sequence runs). |
