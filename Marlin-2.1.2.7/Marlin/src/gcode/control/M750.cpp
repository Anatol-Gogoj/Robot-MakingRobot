...
// Outcome of the measure phase. Two abnormal exits need two different reports:
// an operator abort (M410 / sequencer Stop) is an orderly STOPPED, while a dead
// ODrive link or an empty sample set is an error (issue #42).
enum MeasureResult : uint8_t { MEASURE_OK, MEASURE_ABORTED, MEASURE_FAILED };
 ...
 */
static MeasureResult measureSpeed(const int dur_s) {
  ...
  while (millis() - startTime < duration) {
      ...
      if (Spincoater::abortRequested) return MEASURE_ABORTED;   // RMR: aborted from the UI (M410 / seq Stop)
    #endif
...
void GcodeSuite::M750() {
  ...
  const MeasureResult mr = measureSpeed(dur_s);
  if (mr == MEASURE_ABORTED) {   // RMR: aborted from the UI during the hold (M410 / sequencer Stop)
    Spincoater::abortRequested = false;
    Spincoater::setVelocity(0);
    SERIAL_ECHOLNPGM("echo:SPIN STATE:STOPPED");
    SERIAL_ECHOLNPGM("echo:SPIN OK: ABORTED");
  ...
  if (mr == MEASURE_FAILED) {    // link lost or no samples (issue #42): an error, not a stop
