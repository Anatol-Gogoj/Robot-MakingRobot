...
| Y           | Y             | Gantry Y        | DM556T #2  | A6 (60)   | A7 (61)   | A2 (56)   | 57.14    | 150         | GT2-14 pulley; **2 motors wired in parallel to a single driver** |
| Y2          | —             | Gantry Y clone  | (shared)   | 36        | 34        | 30        | —        | —           | Declared via `Y2_DRIVER_TYPE` so Marlin pulses a second pin set, but this set is redundant — both motors are spliced to the one physical driver. Y2 pins may be left unwired. Auto-squaring (`Y_DUAL_ENDSTOPS`) is NOT possible with this wiring. |
