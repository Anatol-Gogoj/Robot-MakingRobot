...
- **Defaults and priming.** The Syringe block's page defaults are now the 150 mL syringe values with a 120 mm stroke; an **Initial purge +** button pushes a user-set increment (0.5 mL) with the tip where it is, for priming a fresh barrel; all five sequencer cards open by default.
